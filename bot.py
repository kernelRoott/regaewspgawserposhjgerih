...
async def cmd_reset_data(message: Message):
    """Обработчик команды /reset_data (только для администратора)"""
    ...
    try:
        ...
        await message.answer('✅ База данных очищена. Все данные сброшены.')
        logger.info(f"Администратор {message.from_user.id} сбросил базу данных")
    except Exception as e:
